...
struct MapsView: View {
    ...
    @State private var isPitchToggled = false // 피치 토글 상태
...
//            }
        //MARK: 올라오는 모달 : 네잎클로바 선택시
            .sheet(item: $selectedSpecialAnnotation) { annotation in
                    ModalView(annotation: convertToPointAnnotation(annotation), selectedSpecialAnnotation: $selectedSpecialAnnotation)
...
//            }
    ...
    // MKPointAnnotation으로 변환하는 함수
        ...
        // 모달을 표시하는 함수
        func showModal(for annotation: AnnotationItem) {
            let pointAnnotation = convertToPointAnnotation(annotation)
            selectedSpecialAnnotation = annotation
            // 여기서 모달을 호출합니다.
            ModalView(annotation: pointAnnotation, selectedSpecialAnnotation: $selectedSpecialAnnotation)
        }
}
